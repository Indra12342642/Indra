...
let tamnel = fs.readFileSync('./media/indrabot.jpg')
...
🛒 𝗦𝗲𝗹𝗹 𝗜𝗻𝘃𝗲𝗻𝘁𝗼𝗿𝘆
 ...
 • selldiamond`)
        ...
        //+++ || FAKE TROLI         
        const troli =  {key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 2022, status: 200, thumbnail: tamnel, surface: 200, message: `ELAINABOTZ`, orderTitle: 'LordIndra', sellerJid: '0@s.whatsapp.net'} } }    

        const katalog = (teks) => {
             res = zee.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 999999999, "message": teks, "footerText": "©indrabot", "jpegThumbnail": fs.readFileSync('./media/indrabot.jpg'), "surface": 'CATALOG' }}, {quoted:troli})
...
//»»»𝘉𝘶𝘵𝘵𝘰𝘯 𝘉𝘢𝘮𝘩 
    ...
    case 'group': case 'gc': case 'grup':     	 
          ...
          sendButton(from, menu, 'Created By Indra', but, troli)
          break           
...
//»»»𝘖𝘸𝘯𝘦𝘳 𝘔𝘦𝘯𝘶
	...
	case 'setthumb':
		  ...
		  fs.writeFileSync(`./media/indrabot.jpg`, delb)
		  reply('Sukses')
          } else {
...
//»»»𝘔𝘢𝘬𝘦𝘳 𝘔𝘦𝘯𝘶          
  // Textprome //
    ...
    case 'horrorblood':   case 'thunder':
          if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(mess.limit)
          if (args.length == 0) return reply(`Example: ${prefix + command} indrabot`)
          ini_txt = args.join(" ")
...
${q+=1}.term`)        
          ...
          sendButton(from, pp, 'Created By Indra', but, troli)
          break         
...
vcard3 = 'BEGIN:VCARD\n' +
            ...
            `item5.X-ABLabel:⚔️ Elaina - Botz Owner\n` +
            'END:VCARD'.trim()
zee.sendMessage(from, {displayName: `Creator Indra`, vcard: vcard3}, contact, 
{ quoted: troli, 
})
break
   ...
   default:
          ...
          sendButton(from, menu, 'Created By Indra', but, mek)
          break
          }
